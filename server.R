...
shinyServer(function(input, output) {
  output$table <- renderDataTable({
    ...
    #mtcars2<- mtcars[grep("^Merc|Maserati|Cadillac|Lincoln|Porsche|Maserati",rownames(mtcars)),]
    
    mtcars3<- mtcars[grep("^Merc",rownames(mtcars)),]
    
    data <- transmute(mtcars3, Car = rownames(mtcars3), MilesPerGallon = mpg,
    ...
    data
  }, options = list(lengthMenu = c(3, 17, 33), pageLength = 50))
})
